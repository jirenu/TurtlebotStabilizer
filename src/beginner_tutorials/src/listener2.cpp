...
float rewardCalculator(StateReward st)
{
	...
	float maxReward = 10;
        float beta = 0.4;
        float lossImuSquared = pow(100*st.imu1LinearX - 100*st.imu2LinearX, 2) +
                               pow(100*st.imu1LinearZ - 100*st.imu2LinearZ, 2);
        float lossImu = pow(lossImuSquared, 0.5);
        float lossTeleopSquared = pow(100*st.teleopLinearXIn - 100*st.teleopLinearXOut, 2) +
                               pow(100*st.teleopAngularZIn - 100*st.teleopAngularZOut, 2);
...
{
  stateReward.imu1LinearX = cancelNoise(imu1.linear_acceleration.x, 0.0, -0.22);
  //stateReward.imu1LinearY = imu1.linear_acceleration.y;
  stateReward.imu1LinearZ = cancelNoise(imu1.linear_acceleration.z,0.0, -0.23);
